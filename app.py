...
import time
import docx  # --- NEW: Added import for processing .docx files ---
...
def login_page():
    ...
    with st.form(key="login_form"):
        ...
        # 2. Use a form_submit_button instead of a regular button
        submit_button = st.form_submit_button("Login ✅")

    # 3. Check if the submit button was clicked (or Enter was pressed)
    if submit_button:
...
try:
    ...
    st.error("GEMINI_API_KEY not found in Streamlit secrets. Please add it to run the app.")
    st.stop()
except Exception as e:
...
def get_custom_css(theme="Light"):
    ...
    .stTextInput input, .stTextArea textarea, .stSelectbox div[data-baseweb="select"] > div {{
        ...
        /* display:hidden; */ /* You had this, I commented it out as it hides the inputs */
    }}
    /* Ensure selectbox dropdown items are themed */
    div[data-baseweb="popover"] ul li {{
        background-color: {input_bg} !important;
        color: {input_text} !important;
    }}
    div[data-baseweb="popover"] ul li:hover {{
        background-color: {uploader_hover_bg} !important;
    }}
    ...
    /* Keep the readability fixes from before */
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'EquityTool Chat History', 0, 1, 'C')
        self.ln(10)
...
def generate_chat_pdf(messages):
    ...
    if os.path.exists(font_path):
        try:
            # FIX 1: Register the Unicode font
    ...
    # OLD: pdf.cell(0, 7, "You:", ln=1)
# NEW:
    pdf.cell(0, 7, "You:", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(5)
    ...
    # --- OUTPUT ---
    # FIX 3: Simply return the output of dest='S'. 
    # The traceback confirms this returns a byte array/bytes, which Streamlit needs.
    # The Unicode characters will be correctly embedded if NotoSans is present.
    try:
        # FIX 1: The FPDF method returns a bytearray/bytes, but Streamlit prefers the standard 'bytes' type.
        # We explicitly convert the output to standard bytes.
        # This will resolve the StreamlitAPIException: Invalid binary data format: <class 'bytearray'>
        pdf_bytearray = pdf.output(dest='S')
        return bytes(pdf_bytearray)
        
    except Exception as e:
        # Catch any unexpected errors during output
        raise RuntimeError(f"Critical PDF output error: {e}")
...
# --- MODIFIED FUNCTION ---
def ask_question_with_gemini(content_text, question, target_language="English"):
    """
    Asks a question to the Gemini model with content and a target language.
    """
    prompt = f"""
You are a helpful research assistant called EquityTool.
Use the following content to answer the question clearly and concisely in {target_language}.
...
with st.sidebar:
    ...
    if st.button("Logout 🚪", use_container_width=True):
        st.session_state.page = "login"
        st.session_state.username = ""
        # Clear chat and content on logout
        st.session_state.messages = []
        st.session_state.content_loaded = False
        st.session_state.total_content = ""
    ...
    if resource_type == "📄 Upload Document":
        ...
        uploaded_file = st.file_uploader(
            "Choose a file",
            # --- MODIFIED: Added "docx" to type list ---
        ...
        if uploaded_file and st.button("📊 Process Document", type="primary", use_container_width=True):
            with st.spinner("Processing document..."):
                ...
                # --- NEW: Added logic for .docx files ---
                elif uploaded_file.type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
                    content_text = extract_text_from_docx(uploaded_file)
                    st.session_state.content_source = f"📄 DOCX: {uploaded_file.name}"
                # --- END NEW LOGIC ---
        ...
        if urls and st.button("🌐 Fetch All Articles", type="primary", use_container_width=True):
            content_text = ""
        ...
        if direct_text and st.button("📝 Use Text", type="primary", use_container_width=True):
    ...
    languages = [
        "Assamese", "Bengali", "English", "French", "German", "Gujarati", "Hindi", "Japanese", "Kannada", "Konkani", "Korean", "Malayalam", "Mandarin", "Marathi", "Portuguese", "Punjabi", "Russian", "Sanskrit", "Spanish", "Tamil", "Telugu", "Urdu"
    ]
    if "output_language" not in st.session_state:
        st.session_state.output_language = "English" # Default
    
    # Update session state directly from selectbox
...
def handle_submit():
    if st.session_state.question.strip():
        ...
        with st.spinner("EquityTool is thinking..."):
            ...
            target_language = st.session_state.get("output_language", "English")
            answer = ask_question_with_gemini(
                st.session_state.total_content, 
                question, 
                target_language
            )
        ...
        # --- FIX: Removed st.rerun() here ---
        # The script will rerun automatically after this callback finishes.
...
st.markdown(f"""
<div class="footer">
    Made by BotBuddies | {theme_emoji} {st.session_state.theme} Theme | {lang_emoji} {st.session_state.output_language} Output
</div>
""", unsafe_allow_html=True)